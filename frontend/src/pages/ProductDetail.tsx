...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const result = await apiFetch<{ product: Product }>('/api/products/' + id);
        ...
        setRelated(
          (productsResult.products ?? []).filter((p) => p.category === result.product.category && p.id !== result.product.id).slice(0, 4),
        );
        setError(null);
      } catch (err) {
        if (!active) return;
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-7xl px-6 py-20 text-center">
        <p className="font-serif text-2xl text-royal-900">Loading product...</p>
      </div>
    );
  }
  ...
  if (error || !product) {
    return (
      <div className="mx-auto max-w-7xl px-6 py-20 text-center">
        ...
        <p className="mt-2 text-royal-500">{error ?? 'This product could not be loaded.'}</p>
